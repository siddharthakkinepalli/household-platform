...
class WeightedReceiptRefiner : ReceiptRefiner {
    ...
    override fun refine(visionText: VisionTextPayload): RefinedScan {
        val normalizedLines = visionText.lines
            .map { it.trim() }
            .filter { it.isNotBlank() }

        val amountCandidates = normalizedLines
            .mapIndexed { index, line ->
                val parsed = parseAmount(line) ?: return@mapIndexed null
                val score = scoreAmountLine(line = line, index = index)
        ...
        val bestMerchant = selectMerchant(normalizedLines)
    ...
    private fun scoreAmountLine(line: String, index: Int): Float {
        val lower = line.lowercase()
        var score = 0.42f
        if (containsAmountAnchor(lower)) score += 0.42f
        if (lower.contains("eur") || lower.contains("euro") || lower.contains("€")) score += 0.10f
        if (line.count { it.isDigit() } >= 3) score += 0.06f
        if (index >= 3) score -= 0.06f
        return score.coerceAtMost(1f)
    }

    private fun selectMerchant(lines: List<String>): MerchantCandidate? {
        return lines
            .mapIndexed { index, line ->
                val normalized = normalizeMerchantLine(line)
                val score = scoreMerchantLine(line = normalized, index = index)
                MerchantCandidate(value = normalized, score = score)
            }
    ...
    private fun scoreMerchantLine(line: String, index: Int): Float {
        val lower = line.lowercase()
        var score = 0.22f

        if (index <= 2) score += 0.32f
        if (line.none { it.isDigit() }) score += 0.18f
        if (uppercaseRatio(line) >= 0.6f) score += 0.14f
        if (line.length in 4..38) score += 0.08f
        if (merchantSuffixes.any { lower.contains(it) }) score += 0.20f

        if (containsAmountAnchor(lower)) score -= 0.40f
        if (parseAmount(line) != null) score -= 0.45f
        if (parseDate(line) != null) score -= 0.36f
